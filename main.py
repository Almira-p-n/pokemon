...
from logic import Pokemon, Penyihir, Petarung
...
async def go(ctx):
    ...
    if author not in Pokemon.pokemons:  # Periksa apakah pengguna ini memiliki Pokémon 
        ...
        if chance == 1:
            ...
            pokemon = Penyihir(author)  # Membuat Pokémon Wizard 
        elif chance == 3:
            pokemon = Petarung(author)  # Membuat Pokémon Fighter 
...
async def ability(ctx):
    ...
    if author in Pokemon.pokemons.keys():
        ...
        await ctx.send(await pokemon.get_ability())
    else:
